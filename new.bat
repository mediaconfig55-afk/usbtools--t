...
:: ===============================
:: USBTOOLS - GITHUB MAIN FILE
:: ===============================
if defined USBTOOLS_LOADED goto START
set USBTOOLS_LOADED=1
...
echo  --- PRO TEKNISYEN ARACLARI ---            [34] WIFI Sifresi Goster
echo  [36] Windows Update Servislerini SIFIRLA  [35] CIKIS
echo  [37] Tum Suruculeri YEDEKLE               ----------------------------------------------
echo  [38] Guc Plani: NIHAI PERFORMANS          [41] Explorer ONAR
echo  [39] Hosts Dosyasini SIFIRLA              [42] BIOS / Anakart Bilgisi
echo  [40] Yazici Kuyrugunu TEMIZLE             [43] Pil Saglik Raporu
...
:RESET_UPDATE
cls & net stop wuauserv & net stop cryptSvc & net stop bits & net stop msiserver
...
:BACKUP_DRIVERS
cls & mkdir "%USERPROFILE%\Desktop\Driver_Yedek"
...
:RESET_HOSTS
cls & set "hosts=%windir%\system32\drivers\etc\hosts"
attrib -r -s -h %hosts%
...
:CLEAR_SPOOLER
cls & net stop spooler & del /Q /F /S "%systemroot%\System32\Spool\Printers\*.*" & net start spooler
pause & goto MENU
...
:BATTERY_REPORT
cls & powercfg /batteryreport /output "%USERPROFILE%\Desktop\Pil_Raporu.html"
start "" "%USERPROFILE%\Desktop\Pil_Raporu.html"
pause & goto MENU

:DISABLE_TELEMETRY
cls & sc delete DiagTrack & sc delete dmwappushservice
...
:CLEAN_TEMP_FILES
cls & del /q /f /s %TEMP%\* & del /q /f /s %WINDIR%\Temp\* & pause & goto MENU

:WIFI_PASSWORD
cls & set /p wifi="Ag Adi: " & netsh wlan show profile name="%wifi%" key=clear | findstr /i "Key Content"
pause & goto MENU

:CLEAR_DNS
ipconfig /flushdns & pause & goto MENU

:UPDATE_PROGRAMS
winget upgrade --all & pause & goto MENU

:UPDATE_STORE_APPS
powershell -Command "Get-AppxPackage | Foreach {Add-AppxPackage -Path $_.InstallLocation -Register \"$($_.InstallLocation)\AppXManifest.xml\" -DisableDevelopmentMode}"
pause & goto MENU
...
:OPTIMIZE_RAM
cls & start /wait rundll32.exe advapi32.dll,ProcessIdleTasks & pause & goto MENU
